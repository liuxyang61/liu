<!doctype html>
    ...
    <head>
        ...
        <link src="../org/bootstrap.min.css">
        <script src="../org/angular.min.js"></script>
        <title>Document</title>
    </head>
    <body ng-app="hd" ng-controller="ctrl" style="">
<!--    服务协议内容开始-->
    <div style="position: relative;background:white; width: 960px; margin:0 auto; text-align: center; margin-bottom: 100px;padding: 20px">
            <img style="width: 70px;height: 70px;position: absolute;left: 20px; top: 20px;  " src="1.png" alt="后盾网">
<!--        标题-->
        <h2 style="text-align: center;color: red;margin-left: 200px">后盾IT教育会员中心服务协议  <a href="http://cms.liuxyang.com" style="margin-left: 100px ;font-size: 14px;text-decoration:none;line-height: 1.5em;color:#E70014" ><<前往站长博客</a></h2>

        <hr style="margin-top: 30px">
<!--        副标题-->
        <p>请仔细阅读后盾IT教育会员中心服务协议内容</p>
<!--        具体内容-->
        <div style="padding: 10px; text-align: left"  ng-if='good.show' >
            <div class="status" style="font-size: 14px;line-height: 1.5em"><br/>
                <div class="protocol">
                    <h3 align="left">用户使用协议</h3>
                    <P>
                    <h4>一、总则</h4>
                    1.1  后盾网的所有权和运营权归北京市后盾IT教育有限公司所有。 <br/>
                    1.2  用户在注册之前，应当仔细阅读本协议，并同意遵守本协议后方可成为注册用户。一旦注册成功，则用户与后盾网之间自动形成协议关系，用户应当受本协议的约束。用户在使用特殊的服务或产品时，应当同意接受相关协议后方能使用。  <br/>
                    1.3 本协议则可由后盾网随时更新，用户应当及时关注并同意本站不承担通知义务。本站的通知、公告、声明或其它类似内容是本协议的一部分。
                    </P>
                    <p>
                    <h4>二、服务内容</h4>
                    2.1 后盾网的具体内容由本站根据实际情况提供。 <br/>
                    2.2 本站仅提供相关的网络服务，除此之外与相关网络服务有关的设备(如个人电脑、手机、及其他与接入互联网或移动网有关的装置)及所需的费用(如为接入互联网而支付的电话费及上网费、为使用移动网而支付的手机费)均应由用户自行负担。
                    </p>
                    <P>
                    <h4>三、用户帐号</h4>
                    3.1 经本站注册系统完成注册程序并通过身份认证的用户即成为正式用户，可以获得本站规定用户所应享有的一切权限；未经认证仅享有本站规定的部分会员权限。后盾网有权对会员的权限设计进行变更。 <br/>
    ...
    (4)为维护社会公众的利益。
                    ...
                    6.3 使用本站的任何内容均应注明“来源于后盾网”及署上作者姓名，按法律规定需要支付稿酬的，应当通知本站及作者及支付稿酬，并独立承担一切法律责任。<br/>
                    ...
                    <p>
                    <h4>七、责任声明</h4>
                    7.1 用户明确同意其使用本站网络服务所存在的风险及一切后果将完全由用户本人承担，后盾网对此不承担任何责任。 <br/>
...
<!--        表单-->
        <form action="" >
<!--            复选框-->
            <input type="checkbox" ng-model="good.ti" ng-true-value="0" ng-false-value="1"><span style="font-size: 14px;line-height: 1.5em">我已阅读</span>
<!--            复选框结束-->

<!--            服务链接-->
            <a style="text-decoration:none;font-size: 14px;line-height: 1.5em;color:green" href="javascript:;" ng-model="good.show" ng-click="good.func()"><<后盾IT教育会员中心服务协议内容>></a>
<!--            服务链接结束-->
            <!--    {{show}}-->
            <!--    协议内容-->

            <!--    ng-disabled='true'的时候，处于禁止状态-->
<!--            按钮-->
            <div >
                <input   type="submit" ng-disabled="good.ti" value="我已同意">
            </div>
        </form>
<!--        表单结束-->
    </div>
    ...
    <script>
        angular.module('hd',[]).controller('ctrl',['$scope',function($scope){
            $scope.good ={
                show:0,
                ti:1,
                func:function () { $scope.good.show==1?$scope.good.show=0:$scope.good.show=1;}
            }
        }])
    </script>
    </html>